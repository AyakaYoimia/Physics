...
\end{gather*}
可以取$Q_N$的前若干项进行计算。保留到前两项，
\begin{gather*}
    Q = V^N + \frac{N(N-1)}{2} V^{N-2} \int \d^3 \bm{r}_1 \int \d^3 \bm{r}_2 f(r_{12})
    \\
    = V^N + \frac{N(N-1)}{2} V^{N-2} \int f(r) \d^3 \bm{r} = V^N - N(N-1) V^{N-1} a_2(T)
    \\
    a_2(T) = -\frac{1}{2} \int f(r) \d^3 \bm{r} = -2\pi \int_0^{\infty} \left(\e^{\beta\phi(r)}-1\right) r^2 \d r
\end{gather*}
